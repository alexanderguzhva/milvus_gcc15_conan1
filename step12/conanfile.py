...
#        "yaml-cpp/0.7.0#9c87b3998de893cf2e5a08ad09a7a6e0",
    ...
    def configure(self):
        ...
        if self.settings.os == "Macos":
            ...
            self.requires("thrift/0.22.0", override=True)

    def requirements(self):
        if self.settings.os != "Macos":
            self.requires("libunwind/1.7.2")
        #self.requires("yaml-cpp/0.8.0", override=True)
        self.requires("cyrus-sasl/2.1.28.custom", override=True)
        #self.requires("googleapis/cci.20230501", override=True)
        self.requires("boost/1.85.0", override=True)
        self.requires("libevent/2.1.12", override=True)
        self.requires("thrift/0.22.0", override=True)
        #self.requires("google-cloud-cpp/2.28.0", override=True)
